...
> **Active research track:** MedShiftLab-CXR. The existing NeuroSight material is retained below as legacy project history and engineering context.

## MedShiftLab-CXR: Active Research Track

MedShiftLab-CXR is a focused research framework for **data-centric evaluation of pretrained chest X-ray foundation models under annotation uncertainty and cross-dataset distribution shift**. It is designed for reproducible PhD-application research, not clinical use, and does not claim clinical validity.

### Research Scope

- **Internal dataset:** CheXpert, including explicit uncertainty-label strategies.
- **External validation dataset:** VinDr-CXR, kept separate from tuning and model selection.
- **Future scope only:** MIMIC-CXR and CT/MRI extensions are not part of the current implementation.

The active track currently implements:

- label ontology and uncertainty handling
- validated metadata and data-record layers
- label-wise evaluation metrics and structured reports
- a model adapter protocol and dependency-safe optional TorchXRayVision boundary
- prediction-to-evaluation matching by `image_id`
- in-memory and file-exporting experiment runners
- reproducible JSON and CSV report export utilities

Current report artifacts are:

- `evaluation_report.json`: complete run metadata, aggregate metrics, and label-wise metrics
- `evaluation_label_metrics.csv`: one stable row per evaluated label

The current boundary is intentionally narrow: there is no real model inference or image loading yet, no dataset or model-weight downloading, no model training, no threshold tuning, and no calibration fitting. Frontend and API code belong to the legacy NeuroSight scaffold and are not part of the active MedShiftLab-CXR research track.
...
This is the focused test boundary for the active research track; see the [CI/test boundary](docs/medshiftlab/ci_test_boundary.md) for scope and exclusions.

### Research Documentation

- [Research protocol](docs/medshiftlab/research_protocol.md)
- [Data layer design](docs/medshiftlab/data_layer_design.md)
- [Evaluation layer design](docs/medshiftlab/evaluation_layer_design.md)
- [Model adapter layer design](docs/medshiftlab/model_adapter_layer_design.md)
- [Experiment pipeline design](docs/medshiftlab/experiment_pipeline_design.md)
