...
Confidence intervals use a prespecified percentile bootstrap with 2,000 replicates. Resampling is performed at patient level when complete patient identifiers are available so that all images from one patient remain together. If patient identifiers are unavailable or incomplete, image-level resampling may be used only with that limitation stated. The random seed, software version, requested and valid replicate counts, and resampling unit must be recorded. Phase 10 implements deterministic percentile intervals for the existing scalar label metrics. The manual CLI defaults to zero replicates so expensive local analysis remains explicit; a frozen confirmatory run must explicitly request the protocol-defined 2,000 replicates.

## 6. Calibration analysis

Calibration analysis includes label-wise Brier score, ECE, and calibration curves. ECE and calibration curves use 10 equal-width probability bins over `[0, 1]` unless a future protocol version is frozen before evaluation. Empty-bin handling and bin counts must be recorded. Phase 10 exports the underlying per-bin counts, mean scores, mean targets, and absolute calibration gaps, including explicit empty bins. It does not fit a calibrator or render calibration figures.
...
- **Phase 10:** add manual-only analysis over standardized prediction/evaluation artifacts: calibration-bin tables, deterministic percentile bootstrap intervals for supported scalar metrics, subgroup metrics with coverage and skip accounting, and machine-readable non-clinical failure flags.
- **Current robustness-analysis boundary:** `scripts/run_robustness_calibration_analysis.py` is bounded by default and requires an explicit override for large local runs. Patient-cluster bootstrap is used only when patient identifiers are complete; otherwise the report records sample-level resampling. Unsupported metrics and missing requested subgroup columns fail clearly. Calibration plotting and fitting remain unimplemented. No real robustness conclusion, full-dataset evaluation, external-validation result, benchmark completion, clinical validation, training/fine-tuning, or SOTA claim is introduced.
